...
import { Dumbbell, Home, Utensils, Brain, BarChart2 } from 'lucide-react';
...
const BottomNavbar = ({ currentPage }: BottomNavbarProps) => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-background border-t py-2 px-4 flex justify-between">
      {navItems.map((item) => (
        <button
          key={item.label}
          className={`flex flex-col items-center p-1 ${currentPage === item.label.toLowerCase() ? 'text-primary' : 'text-muted-foreground'}`}
